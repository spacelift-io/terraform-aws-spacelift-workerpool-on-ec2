...
variable "autoscaling_configuration" {
  ...
  - ca_bundle: (optional) PEM-encoded certificate or certificate chain to trust when the autoscaler connects to the Spacelift API. Useful for self-hosted deployments with a private CA. Example: file("ca.pem") or the PEM string directly.
  EOF
