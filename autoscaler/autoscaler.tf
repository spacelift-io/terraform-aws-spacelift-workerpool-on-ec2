...
resource "aws_ssm_parameter" "spacelift_api_key_secret" {
  ...
  type  = "SecureString"
  value = var.autoscaling_configuration.api_key_secret
}
...
resource "null_resource" "download" {
  ...
  triggers = {
    ...
    keeper = local.autoscaler_version == "latest" ? timestamp() : local.autoscaler_version
  }
  provisioner "local-exec" {
    command = "${path.module}/download.sh \"${local.autoscaler_version}\" \"${var.autoscaling_configuration.architecture}\""
  }
}
...
data "archive_file" "binary" {
  ...
  output_path = "ec2-workerpool-autoscaler_${local.autoscaler_version}.zip"
...
resource "aws_lambda_function" "autoscaler" {
  ...
  filename         = !local.use_s3_package ? data.archive_file.binary[0].output_path : null
  source_code_hash = !local.use_s3_package ? data.archive_file.binary[0].output_base64sha256 : null
  ...
  environment {
    variables = {
      ...
      SPACELIFT_API_KEY_ID          = var.autoscaling_configuration.api_key_id
      SPACELIFT_API_KEY_SECRET_NAME = aws_ssm_parameter.spacelift_api_key_secret.name
      SPACELIFT_API_KEY_ENDPOINT    = var.autoscaling_configuration.api_key_endpoint
